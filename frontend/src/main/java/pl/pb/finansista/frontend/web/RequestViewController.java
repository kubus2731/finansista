...
public class RequestViewController {
  ...
  public String list(
    ...
    List<RequestResponse> responses =
        backendRestClient
            ...
            .uri(
                b ->
                    b.path("/api/v1/requests")
                        .queryParamIfPresent(
                            "status",
                            Optional.ofNullable(StringUtils.hasText(status) ? status : null))
                        .queryParamIfPresent(
                            "search",
                            Optional.ofNullable(StringUtils.hasText(search) ? search : null))
                        .build())
            .header("Authorization", bearer(httpRequest))
            .retrieve()
            .body(new ParameterizedTypeReference<List<RequestResponse>>() {});

    List<RequestView> views =
        (responses == null ? List.<RequestResponse>of() : responses)
  ...
  public ResponseEntity<byte[]> downloadAttachment(
      @PathVariable String id, @PathVariable String aid, HttpServletRequest httpRequest) {
    ...
    return new ResponseEntity<>(
        backendResponse.getBody(), headers, backendResponse.getStatusCode());
  }
  ...
  public String changeStatus(
    ...
    try {
      ...
      redirectAttributes.addFlashAttribute(
          "errorMessage", errorResolver.resolve(e, "Nie udało się zmienić statusu wniosku"));
    }
    return "redirect:/requests/" + id;
  }
  ...
  public String createForm(
      Authentication authentication, HttpServletRequest httpRequest, Model model) {
  ...
  public String update(
    ...
    List<EditRequestRequest.CostItemEntry> costItems =
        form.getCostItems().stream()
            .filter(c -> StringUtils.hasText(c.getItemName()))
            .map(
                c ->
                    new EditRequestRequest.CostItemEntry(
                        c.getTaskNo(),
                        c.getItemName(),
                        c.getQuantity(),
                        c.getUnitCost(),
    ...
    List<EditRequestRequest.FundingEntry> fundings =
        form.getFundings().stream()
            .filter(f -> f.getAmountRequested() != null)
            .map(
                f ->
                    new EditRequestRequest.FundingEntry(
                        fundingSourceIdFromName(f.getSourceName()), f.getAmountRequested()))
            .toList();

    EditRequestRequest payload =
        new EditRequestRequest(
            ...
            form.getRealizerType(),
            form.getProjectKind(),
            form.getProjectKindOther(),
            form.getProjectScope(),
            form.getProjectScopeOther(),
            form.getProjectNature(),
            form.getProjectNatureOther(),
            form.getPlannedDateFrom(),
            form.getPlannedDateTo(),
            form.getLocation(),
            form.getParticipantsInvolved(),
            form.getParticipantsBenefiting(),
            form.getSupervisorName(),
            form.getSupervisorEmail(),
            form.getSupervisorPhone(),
            form.getSupervisorDepartment(),
            tasks,
            costItems,
            fundings);
  ...
  public String create(
    ...
    List<CreateRequestRequest.TaskItem> tasks =
        form.getTasks().stream()
            .filter(t -> StringUtils.hasText(t.getName()) || t.getPlannedCost() != null)
            .map(
                t ->
                    new CreateRequestRequest.TaskItem(
                        t.getTaskNo(),
                        t.getName(),
                        t.getDateFrom(),
                        t.getDateTo(),
    ...
    List<CreateRequestRequest.CostItemEntry> costItems =
        form.getCostItems().stream()
            .filter(c -> StringUtils.hasText(c.getItemName()))
            .map(
                c ->
                    new CreateRequestRequest.CostItemEntry(
                        c.getTaskNo(),
                        c.getItemName(),
                        c.getQuantity(),
                        c.getUnitCost(),
    ...
    List<CreateRequestRequest.FundingEntry> fundings =
        form.getFundings().stream()
            .filter(f -> f.getAmountRequested() != null)
            .map(
                f ->
                    new CreateRequestRequest.FundingEntry(
                        fundingSourceIdFromName(f.getSourceName()), f.getAmountRequested()))
            .toList();

    CreateRequestRequest payload =
    ...
    try {
      ...
      redirectAttributes.addFlashAttribute(
          "successMessage", "Wniosek został zapisany jako wersja robocza.");
      return "redirect:/requests";
    } catch (RestClientResponseException e) {
      model.addAttribute("departmentName", user.departmentName());
      model.addAttribute(
          "validationErrors", List.of(errorResolver.resolve(e, "Nie udało się zapisać wniosku")));
      return "requests/form";
    }
  }
  ...
  private CreateRequestForm toForm(RequestResponse r) {
    ...
    List<RequestResponse.CostItemResponse> srcCosts =
        r.costItems() == null ? List.of() : r.costItems();
  ...
  private CreateRequestForm.FundingRow fundingRowWithAmount(
      String sourceName, Long sourceId, RequestResponse r) {
  ...
  private List<String> validateBusinessRules(CreateRequestForm form) {
    ...
    if (form.getPlannedDateFrom() != null
        && form.getPlannedDateTo() != null
    ...
    for (CreateRequestForm.TaskRow t : form.getTasks()) {
      if (t.getDateFrom() != null
          && t.getDateTo() != null
          && t.getDateTo().isBefore(t.getDateFrom())) {
        errors.add(
            "Zadanie nr "
                + t.getTaskNo()
                + ": data zakończenia jest wcześniejsza niż rozpoczęcia.");
      }
    ...
    if (anyFunding && form.getAmount() != null && fundingSum.compareTo(form.getAmount()) != 0) {
      errors.add(
          "Suma kwot ze źródeł ("
              + fundingSum
              + " zł) musi równać się łącznej wnioskowanej kwocie ("
              + form.getAmount()
              + " zł).");
    }
