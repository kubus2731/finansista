package pl.pb.finansista.frontend.common;
...
public class ExternalIdEncoder {
    ...
    public static UUID decode(String encodedId) {
        ...
        try {
            ...
            if (bytes.length != 16) throw new BusinessException("Nieprawidłowy identyfikator");

            ByteBuffer bb = ByteBuffer.wrap(bytes);
            return new UUID(bb.getLong(), bb.getLong());
        } catch (IllegalArgumentException e) {
            throw new BusinessException("Nieprawidłowy identyfikator");
        }
    }
}
