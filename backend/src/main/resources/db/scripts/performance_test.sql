...
-- Testy wydajnościowe tabeli transakcyjnej REQUESTS

-- Sprawdzenie skali danych (liczba wierszy przed testem)
SELECT COUNT(*) AS wierszy_w_requests FROM requests;

-- Q1: Paginacja listy wniosków (strona 3 po 20, najnowsze)
SELECT id, title, amount, created_at
FROM requests
ORDER BY created_at DESC
OFFSET 40 ROWS FETCH NEXT 20 ROWS ONLY;

-- Q2: Wnioski wydziału, sortowane po dacie (ekran przeglądu)
SELECT COUNT(*) AS cnt
FROM (
         SELECT id
         FROM requests
...
-- Q3: Raport agregowany per wydział (widok v_department_requests_summary)
SELECT * FROM v_department_requests_summary ORDER BY department_name;

-- Q4: Wyszukanie wniosku po external_id (wejście z API/linku)
SELECT id, title
FROM requests
WHERE external_id = (SELECT external_id FROM requests WHERE ROWNUM = 1);

-- PLAN WYKONANIA: Q2 (filtr wydziału + sortowanie)
EXPLAIN PLAN FOR
...
-- PLAN WYKONANIA: Q4 (unikalny indeks external_id)
EXPLAIN PLAN FOR
