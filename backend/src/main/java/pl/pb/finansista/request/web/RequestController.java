...
public class RequestController {
  ...
  public ResponseEntity<List<RequestResponse>> getRequests(
      ...
      Authentication authentication) {
    log.info(
        "Fetching requests for user: {} (page={}, size={})",
        userId,
    ...
    GetRequestsQuery query =
        new GetRequestsQuery(userId, authorities, status, departmentId, search);

    Page<Request> page = getRequestsUseCase.execute(query, pageable);
    log.info(
        "Found {} requests on page {} (total elements={}) for user: {}",
  ...
  public ResponseEntity<RequestResponse> editRequest(
    ...
    List<String> authorities =
        authentication.getAuthorities().stream()
            ...
            .collect(Collectors.toList());
    Request request =
        editRequestUseCase.execute(payload.toCommand(id, userId, authorities, version));
  ...
  public ResponseEntity<Void> grantFunding(
    ...
    grantFundingUseCase.execute(
        new GrantFundingCommand(id, fundingSourceId, payload.amountGranted(), userId, authorities));
  ...
  public ResponseEntity<Void> recordProvostOpinion(
    ...
    recordProvostOpinionUseCase.execute(
        new RecordProvostOpinionCommand(id, payload.opinion(), userId, authorities));
    return ResponseEntity.noContent().build();
  }
