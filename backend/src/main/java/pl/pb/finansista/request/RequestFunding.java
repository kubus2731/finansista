 ...
 * realizator; kwotę przyznaną wraz z podpisem (kto/kiedy) wypełnia dysponent danego źródła.
 */
@Entity
@Table(
    ...
    uniqueConstraints =
        @UniqueConstraint(
            name = "uq_reqfunding_request_source",
            columnNames = {"request_id", "funding_source_id"}))
@Getter
...
public class RequestFunding extends BaseEntity {
  ...
  public RequestFunding(Request request, FundingSource source, BigDecimal amountRequested) {
    Assert.isTrue(
        amountRequested == null || amountRequested.signum() >= 0,
