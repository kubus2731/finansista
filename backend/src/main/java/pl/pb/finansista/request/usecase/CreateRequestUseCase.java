  ...
  public Request execute(CreateRequestCommand command) {
    ...
    var costCategory =
        costCategoryRepository
            .findById(command.costCategoryId())
            .orElseThrow(CostCategoryNotFoundException::new);

    RequestTemplate template =
        command.templateId() != null
            ? requestTemplateRepository
                .findByExternalId(command.templateId())
                .orElseThrow(RequestTemplateNotFoundException::new)
            : null;

    RequestStatus status =
        requestStatusRepository
            .findByName(RequestStatusName.DRAFT.name())
            .orElseThrow(
                () -> InvalidRequestStateException.withStatusName(RequestStatusName.DRAFT.name()));
    ...
    command
        ...
        .forEach(
            t ->
                request.addTask(
                    t.taskNo(), t.name(), t.dateFrom(), t.dateTo(), t.plannedCost(), t.actions()));
    command
        .costItems()
        .forEach(
            c ->
