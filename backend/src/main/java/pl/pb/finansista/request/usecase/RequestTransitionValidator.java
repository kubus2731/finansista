 ...
 * GrantFundingUseCase} while UNDER_REVIEW. The request can only reach ACCEPTED once every funding
 * row has been signed by its dysponent.
 *
 * <ul>
 *   <li>SUBMITTED → FORMAL_EVALUATION: CSSDiR (ROLE_STUDENT_AFFAIRS) — ocena formalna
 *   <li>FORMAL_EVALUATION → UNDER_REVIEW: CSSDiR, gated on the recorded opinia prorektora (merit)
 *   <li>UNDER_REVIEW → ACCEPTED: Rektor (ROLE_FINANCE_OFFICE), gated on all rows granted
 * </ul>
 */
@Component
public class RequestTransitionValidator {

  public List<RequestStatusName> getAvailableTransitions(
      Request request, User actor, Collection<String> roles) {
